...
- (id)initWithIdentifier:(NSString *)ident {
    if ((self = [super init])) {
        _keychainItemIdentifier = (UInt8*)malloc(ident.length + 1);
        memset(_keychainItemIdentifier, 0, ident.length + 1);
        memcpy(_keychainItemIdentifier, [ident UTF8String], ident.length);
        ...
        NSData *keychainItemID = [NSData dataWithBytes:_keychainItemIdentifier
                                                length:strlen((const char *)_keychainItemIdentifier)];
...
- (void)resetKeychainItem
{
    ...
    [keychainData setObject:DEFAULT_SECATTRLABEL forKey:(id)kSecAttrLabel];
    [keychainData setObject:DEFAULT_SECSECATTRDESCRIPTION forKey:(id)kSecAttrDescription];
    [keychainData setObject:DEFAULT_SECATTRACCOUNT forKey:(id)kSecAttrAccount];
    [keychainData setObject:DEFAULT_SECATTRSERVICE forKey:(id)kSecAttrService];
    [keychainData setObject:DEFAULT_SECATTRCOMMENT forKey:(id)kSecAttrComment];
    [keychainData setObject:DEFAULT_SECVALUEDATA forKey:(id)kSecValueData];
}
