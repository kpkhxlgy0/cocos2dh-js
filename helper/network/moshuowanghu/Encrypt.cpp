...
#include <stdio.h>
//生成密文
uint16_t CXOREncrypt::EncryptData(const char * pszSrcData, char *pszEncrypData, uint16_t wSize)
{
    //效验参数
    CCAssert(wSize > 0, "");
    CCAssert(pszSrcData != NULL, "");
    CCAssert(pszEncrypData != NULL, "");
    ...
    for (uint16_t i = 1; i < CountArray(szRandKey); i++) szRandKey[i] = rand() % 255;

    //生成密文
    uint8_t bTempCode = 0;
    uint16_t wTimes = (szRandKey[0] + ENCRYPT_KEY_LEN - 1) / ENCRYPT_KEY_LEN * ENCRYPT_KEY_LEN;
    if ((wTimes * 4 + 1) > wSize) return wTimes * 4 + 1;
    for (uint16_t i = 0; i < wTimes; i++)
    {
        if (i < szRandKey[0]) bTempCode = pszSrcData[i] ^ szRandKey[i%ENCRYPT_KEY_LEN];
        else bTempCode = szRandKey[i%ENCRYPT_KEY_LEN] ^ (rand() % 255);
//        _snprintf(pszEncrypData + i * 4, 5, L"%02x%02x", szRandKey[i%ENCRYPT_KEY_LEN], bTempCode);
        sprintf(pszEncrypData + i * 4, "%02x%02x", szRandKey[i%ENCRYPT_KEY_LEN], bTempCode);
    }
...
//解开密文
uint16_t CXOREncrypt::CrevasseData(const char *pszEncrypData, char *pszSrcData, uint16_t wSize)
{
    //效验参数
    CCAssert(wSize > 0, "");
    CCAssert(pszSrcData != NULL, "");
    ...
    //效验长度
    uint16_t wEncrypPassLen = strlen(pszEncrypData);
    if (wEncrypPassLen < ENCRYPT_KEY_LEN * 4) return 0;

    //效验长度
    char *pEnd = NULL;
    char szTempBuffer[3] = {pszEncrypData[0], pszEncrypData[1], 0};
    uint16_t wSoureLength = (uint16_t)strtol(szTempBuffer, &pEnd, 16);
    ...
    for (int i = 0; i < wSoureLength; i++)
    {
        ...
        pszSrcData[i] = (char)((uint8_t)uKey ^ (uint8_t)uEncrypt);
    }
